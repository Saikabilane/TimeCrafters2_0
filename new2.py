        ...
        if "lab"  not in cls :
            ...
            for timeslot, periods in enumerate(schedule):
                if periods!="free":
                    ...
                    if schedule.count((periods[0], periods[1])) > 1:
                        score -= 1  
                    faculty_schedule[periods[1]][timeslot] += 1
                    
        elif "lab" in cls:
            score=0        
    #for work load distribution 
    for fac, schedule in faculty_schedule.items():
...
def mutate(individual):
    for cls in classes:
        if random.random() < MUTATION_RATE:
            crsList = class_courses(cls)
            individual[cls] = ["free" for i in range(NUM_TIMESLOTS)]
            indtimeslot=[x for x in range (NUM_TIMESLOTS)]
            for _ in range(NUM_TIMESLOTS):
                if crsList:
                    randomClass = random.choice(crsList)
                    randomslot=random.choice(indtimeslot)
                    individual[cls][randomslot]=randomClass
                    crsList.remove(randomClass)
                    indtimeslot.remove(randomslot)
...
#print(best_timetable["20PC"]) #this prints the total number of classes for a particular course for the class 20PC as a nested list
