...
# To check if the faculty is assigned multiple classes in a single time slot 
    ...
    for course, distribution in course_distribution.items():
        ...
        if total_slots > expected_slots - 1 and total_slots < expected_slots + 1:
            score += 1  # Reward for good distribution
        else:
    ...
    for cls in classes:
        ...
        for timeslot in range(NUM_TIMESLOTS):
            ...
            for attempt in range(NUM_TIMESLOTS):  # Limiting the number of attempts to NUM_TIMESLOTS
                ...
                conflict = any(
                    timeslot < NUM_TIMESLOTS and
                    timeslot < len(child.get(other_cls, [])) and
                    timeslot < len(selected_parent.get(other_cls, [])) and
                    child.get(other_cls, [])[timeslot] == (selected_parent.get(other_cls, [])[timeslot][0], teacher)
            ...
            for _ in range(NUM_TIMESLOTS):
                if crsList:
                    ...
                    for attempt in range(NUM_TIMESLOTS):  # Limiting the number of attempts to NUM_TIMESLOTS
                        ...
                        if individual[cls][randomslot] == "free" or individual[cls][randomslot][1] != faculty_of_class:
